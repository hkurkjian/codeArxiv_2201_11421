MODULE nrutil
	...
	END INTERFACE
	INTERFACE swap
		MODULE PROCEDURE swap_i,swap_r,swap_d,swap_q,swap_rv,swap_dv,swap_qv,swap_c, &
			swap_cv,swap_cm,swap_z,swap_zv,swap_zm, &
			swap_cq,swap_cqv,swap_cqm, &
	...
	END INTERFACE
	INTERFACE imaxloc
		MODULE PROCEDURE imaxloc_r,imaxloc_d,imaxloc_q,imaxloc_i
	END INTERFACE
        INTERFACE ecrit
                MODULE PROCEDURE ecrit1,ecrit2,ecrit3,ecrit4,ecrit5,ecrit6,ecrit7,ecrit8,ecrit9
        END INTERFACE
	INTERFACE assert
	...
	END INTERFACE
	INTERFACE arth
		MODULE PROCEDURE arth_r, arth_d, arth_i, arth_q
	END INTERFACE
	INTERFACE geop
		MODULE PROCEDURE geop_r, geop_d, geop_q, geop_i, geop_c, geop_cq, geop_dv, geop_qv
	END INTERFACE
	INTERFACE cumsum
		MODULE PROCEDURE cumsum_r,cumsum_i
	END INTERFACE
	INTERFACE poly
		MODULE PROCEDURE poly_rr,poly_dd,poly_qq,poly_rc,poly_rcd,poly_rcq,poly_cc,poly_ccd,poly_ccq,poly_rrv,poly_ddv,poly_qqv,&
			poly_msk_rrv,poly_msk_ddv
	END INTERFACE
	INTERFACE poly_term
		MODULE PROCEDURE poly_term_rr,poly_term_rrd,poly_term_rrq,poly_term_cc,poly_term_ccd,poly_term_ccq
	END INTERFACE
	INTERFACE outerprod
		MODULE PROCEDURE outerprod_r,outerprod_d,outerprod_q,outerprod_cq
	END INTERFACE
	INTERFACE outerdiff
...
!BL
        SUBROUTINE ecrit1(bavard,chaine)
        CHARACTER(LEN=*), INTENT(IN) :: chaine
        LOGICAL, INTENT(IN) :: bavard
        if (bavard) then
                write (*,*) chaine
        end if
        END SUBROUTINE ecrit1
!BL
        SUBROUTINE ecrit2(bavard,chaine,nbr)
        CHARACTER(LEN=*), INTENT(IN) :: chaine
        LOGICAL, INTENT(IN) :: bavard
        REAL(DP), DIMENSION(:), INTENT(IN) :: nbr
        if (bavard) then
                write (*,*) chaine,nbr
        end if
        END SUBROUTINE ecrit2
!BL
        SUBROUTINE ecrit3(bavard,chaine,nbr)
        CHARACTER(LEN=*), INTENT(IN) :: chaine
        LOGICAL, INTENT(IN) :: bavard
        REAL(DP), INTENT(IN)  :: nbr
        if (bavard) then
                write (*,*) chaine,nbr
        end if
        END SUBROUTINE ecrit3
!BL
        SUBROUTINE ecrit4(bavard,chaine,nbr)
        CHARACTER(LEN=*), INTENT(IN) :: chaine
        LOGICAL, INTENT(IN) :: bavard
        REAL(QP), DIMENSION(:), INTENT(IN) :: nbr
        if (bavard) then
                write (*,*) chaine,nbr
        end if
        END SUBROUTINE ecrit4
!BL
        SUBROUTINE ecrit5(bavard,chaine,nbr)
        CHARACTER(LEN=*), INTENT(IN) :: chaine
        LOGICAL, INTENT(IN) :: bavard
        REAL(QP), INTENT(IN)  :: nbr
        if (bavard) then
                write (*,*) chaine,nbr
        end if
        END SUBROUTINE ecrit5
!BL
        SUBROUTINE ecrit6(bavard,chaine,nbr)
        CHARACTER(LEN=*), INTENT(IN) :: chaine
        LOGICAL, INTENT(IN) :: bavard
        COMPLEX(DPC), INTENT(IN)  :: nbr
        if (bavard) then
                write (*,*) chaine,nbr
        end if
        END SUBROUTINE ecrit6
!BL
        SUBROUTINE ecrit7(bavard,chaine,nbr)
        CHARACTER(LEN=*), INTENT(IN) :: chaine
        LOGICAL, INTENT(IN) :: bavard
        COMPLEX(QPC), INTENT(IN)  :: nbr
        if (bavard) then
                write (*,*) chaine,nbr
        end if
        END SUBROUTINE ecrit7
!BL
        SUBROUTINE ecrit8(bavard,chaine,nbr)
        CHARACTER(LEN=*), INTENT(IN) :: chaine
        LOGICAL, INTENT(IN) :: bavard
        COMPLEX(DPC), DIMENSION(:), INTENT(IN)  :: nbr
        if (bavard) then
                write (*,*) chaine,nbr
        end if
        END SUBROUTINE ecrit8
!BL
        SUBROUTINE ecrit9(bavard,chaine,nbr)
        CHARACTER(LEN=*), INTENT(IN) :: chaine
        LOGICAL, INTENT(IN) :: bavard
        COMPLEX(QPC), DIMENSION(:), INTENT(IN)  :: nbr
        if (bavard) then
                write (*,*) chaine,nbr
        end if
        END SUBROUTINE ecrit9
!BL
...
!BL
	...
	END FUNCTION geop_c
!BL
	FUNCTION geop_cq(first,factor,n)
	COMPLEX(QP), INTENT(IN) :: first,factor
	INTEGER(I4B), INTENT(IN) :: n
	COMPLEX(QP), DIMENSION(n) :: geop_cq
	INTEGER(I4B) :: k,k2
	COMPLEX(QP) :: temp
	if (n > 0) geop_cq(1)=first
	if (n <= NPAR_GEOP) then
		do k=2,n
			geop_cq(k)=geop_cq(k-1)*factor
		end do
	else
		do k=2,NPAR2_GEOP
			geop_cq(k)=geop_cq(k-1)*factor
		...
		do
			if (k >= n) exit
			k2=k+k
			geop_cq(k+1:min(k2,n))=temp*geop_cq(1:min(k,n-k))
			temp=temp*temp
			k=k2
		end do
	end if
	END FUNCTION geop_cq
!BL
...
!BL
	...
	if (n <= NPAR_GEOP) then
		...
		do
			if (k >= n) exit
			k2=k+k
			geop_qv(:,k+1:min(k2,n))=geop_qv(:,1:min(k,n-k))*&
				spread(temp,2,size(geop_qv(:,1:min(k,n-k)),2))
			temp=temp*temp
			k=k2
		end do
	end if
...
!BL
	FUNCTION poly_rcd(x,coeffs)
	COMPLEX(DPC), INTENT(IN) :: x
	REAL(DP), DIMENSION(:), INTENT(IN) :: coeffs
	COMPLEX(DPC) :: poly_rcd
	COMPLEX(DPC) :: pow
	...
	END FUNCTION poly_rcd
!BL
	FUNCTION poly_rcq(x,coeffs)
	COMPLEX(QPC), INTENT(IN) :: x
	REAL(QP), DIMENSION(:), INTENT(IN) :: coeffs
	COMPLEX(QPC) :: poly_rcq
	COMPLEX(QPC) :: pow
	COMPLEX(QPC), DIMENSION(:), ALLOCATABLE :: vec
	INTEGER(I4B) :: i,n,nn
	n=size(coeffs)
	if (n <= 0) then
		poly_rcq=0.0_qp
...
!BL
	FUNCTION poly_ccd(x,coeffs)
	COMPLEX(DPC), INTENT(IN) :: x
	COMPLEX(DPC), DIMENSION(:), INTENT(IN) :: coeffs
	COMPLEX(DPC) :: poly_ccd
	COMPLEX(DPC) :: pow
	COMPLEX(DPC), DIMENSION(:), ALLOCATABLE :: vec
	INTEGER(I4B) :: i,n,nn
	n=size(coeffs)
	...
	END FUNCTION poly_ccd
!BL
	FUNCTION poly_ccq(x,coeffs)
	COMPLEX(QPC), INTENT(IN) :: x
	COMPLEX(QPC), DIMENSION(:), INTENT(IN) :: coeffs
	COMPLEX(QPC) :: poly_ccq
	COMPLEX(QPC) :: pow
	COMPLEX(QPC), DIMENSION(:), ALLOCATABLE :: vec
	INTEGER(I4B) :: i,n,nn
	n=size(coeffs)
...
!BL
	RECURSIVE FUNCTION poly_term_rrd(a,b) RESULT(u)
	...
	if (n < NPAR_POLYTERM) then
		...
		u(2:n:2)=poly_term_rrd(a(2:n:2)+a(1:n-1:2)*b,b*b)
		u(3:n:2)=a(3:n:2)+b*u(2:n-1:2)
	end if
	END FUNCTION poly_term_rrd
!BL
	RECURSIVE FUNCTION poly_term_rrq(a,b) RESULT(u)
	...
	if (n < NPAR_POLYTERM) then
		...
		u(2:n:2)=poly_term_rrq(a(2:n:2)+a(1:n-1:2)*b,b*b)
		u(3:n:2)=a(3:n:2)+b*u(2:n-1:2)
	end if
	END FUNCTION poly_term_rrq
!BL
...
!BL
	RECURSIVE FUNCTION poly_term_ccd(a,b) RESULT(u)
	...
	if (n < NPAR_POLYTERM) then
		...
		u(2:n:2)=poly_term_ccd(a(2:n:2)+a(1:n-1:2)*b,b*b)
		u(3:n:2)=a(3:n:2)+b*u(2:n-1:2)
	end if
	END FUNCTION poly_term_ccd
!BL
	RECURSIVE FUNCTION poly_term_ccq(a,b) RESULT(u)
	...
	if (n < NPAR_POLYTERM) then
		...
		u(2:n:2)=poly_term_ccq(a(2:n:2)+a(1:n-1:2)*b,b*b)
		u(3:n:2)=a(3:n:2)+b*u(2:n-1:2)
	end if
	END FUNCTION poly_term_ccq
!BL
!BL
...
!BL
	...
	outerprod_q = spread(a,dim=2,ncopies=size(b)) * &
		spread(b,dim=1,ncopies=size(a))
	END FUNCTION outerprod_q
!BL
	FUNCTION outerprod_cq(a,b)
	COMPLEX(QPC), DIMENSION(:), INTENT(IN) :: a,b
	COMPLEX(QPC), DIMENSION(size(a),size(b)) :: outerprod_cq
	outerprod_cq = spread(a,dim=2,ncopies=size(b)) * &
		spread(b,dim=1,ncopies=size(a))
	END FUNCTION outerprod_cq
!BL
